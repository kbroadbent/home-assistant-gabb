    ...
    async def refresh_access_token(self) -> None:
        ...
        try:
            async with self._session.post(
                ...
                if resp.status == 401:
                    _LOGGER.debug("Refresh token rejected (401), falling back to login")
                    await self.login()
                    return
                data = await self._handle_response(resp)
        except GabbAuthError:
            _LOGGER.debug("Refresh returned auth error, falling back to login")
            await self.login()
            return
        except (GabbConnectionError, GabbAPIError, aiohttp.ClientError, TimeoutError) as err:
            _LOGGER.debug("Refresh failed (%s), falling back to login", err)
            await self.login()
            return
